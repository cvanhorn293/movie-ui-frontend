            ...
            {!selectedGenre ? (
                ...
                <>
                    <div className="mt-1 flex items-center gap-2.5">
                        ...
                        {rank === 1 ? <span className="rounded-full bg-accent-soft px-2.5 py-0.5 text-xs font-medium text-accent">Top genre</span> : rank != null && <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs font-medium text-secondary">#{rank}</span>}
                    </div>
                        ...
                        {topMovies.length === 0 ? (
                            ...
                            <ul className="grid gap-x-5 gap-y-2.5 sm:grid-flow-col sm:grid-cols-2 sm:grid-rows-3">
                                {topMovies.map((movie, index) => {
                                    ...
                                    return (
                                        <li key={movie.tmdbId}>
                                            <Link href={`/movies/${movie.tmdbId}`} className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-white/[0.04]">
