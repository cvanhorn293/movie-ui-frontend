...
export default function DnaStats({ moviesSaved = 0, peopleSaved = 0, avgRating = null, topRating = null, latestYear = null, stats }: DnaStatsProps) {
    const resolvedStats: DnaStatItem[] = stats ?? [
    ...
    const columnsClass = resolvedStats.length <= 4 ? "grid-cols-2 sm:grid-cols-4" : resolvedStats.length === 5 ? "grid-cols-3 sm:grid-cols-5" : "grid-cols-3 sm:grid-cols-6";
