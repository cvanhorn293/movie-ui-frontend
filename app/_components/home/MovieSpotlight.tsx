        ...
        >
            ...
            <div className="relative z-10 flex min-h-[75vh] flex-col justify-end px-4 pt-24 pb-8 sm:px-6 sm:pb-10 md:max-w-[42%] lg:max-w-[38%]">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-3">
                    <Link href={`/movies/${movie.tmdbId}`} className="inline-flex items-center gap-2 rounded-xl bg-primary-gradient px-6 py-3 text-sm font-semibold text-[#04121D] transition-opacity hover:opacity-90">
                        View Movie
                        <ChevRight className="h-4 w-4" />
                    </Link>
                    {isAuthenticated && (
                        <button type="button" onClick={() => toggleFavorite(movie)} aria-pressed={favorited} className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-black/35 px-6 py-3 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:bg-black/50">
